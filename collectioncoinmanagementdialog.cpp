...
CollectionCoinManagementDialog::CollectionCoinManagementDialog(QWidget *parent)
  ...
  ui->listWidgetAvailableCoins->setSelectionMode(
      QAbstractItemView::ExtendedSelection);
  ui->listWidgetTrackedCoins->setSelectionMode(
      QAbstractItemView::ExtendedSelection);
  QObject::connect(
      ui->pushButtonMoveItemsToRight, &QPushButton::clicked, this,
      &CollectionCoinManagementDialog::moveSelectedCoinsToTrackedCoins);
  QObject::connect(
      ui->pushButtonMoveItemToLeft, &QPushButton::clicked, this,
      &CollectionCoinManagementDialog::moveSelectedCoinsToAvailableCoins);
  QObject::connect(ui->pushButtonMoveAllToRight, &QPushButton::clicked, this,
                   &CollectionCoinManagementDialog::moveAllCoinsToTrackedCoins);
  QObject::connect(
      ui->pushButtonMoveAllToLeft, &QPushButton::clicked, this,
      &CollectionCoinManagementDialog::moveAllCoinsToAvailableCoins);

  QObject::connect(ui->buttonControllerBox, &QDialogButtonBox::accepted, this,
                   &CollectionCoinManagementDialog::checkCollectionContents);
  QObject::connect(this, &CollectionCoinManagementDialog::accepted, this,
                   &CollectionCoinManagementDialog::finishedCurrentAction);
  QObject::connect(this, &CollectionCoinManagementDialog::rejected, this,
                   &CollectionCoinManagementDialog::finishedCurrentAction);
}
...
void CollectionCoinManagementDialog::showAddNewCollectionCoin() {
  ...
  if (ui->listWidgetAvailableCoins->count() == 0 &&
      ui->listWidgetTrackedCoins->count() == 0) {
    while (iterQHash.hasNext()) {
      iterQHash.next();
      ui->listWidgetAvailableCoins->addItem(
          iterQHash.value()->getDisplayItem().get());
    }
  } else {
...
void CollectionCoinManagementDialog::showConfigureCurrentCollectionCoin(
    const CoinDataStation &coinDataStation,
    const DataObserver &coinCollection) {
...
void CollectionCoinManagementDialog::setAvailableCoins(
    const QHash<QString, CoinPtr> &availableCoins) {
  trackedCoins = availableCoins;
  emit finishedPreloadAvailableCoins();
}

void CollectionCoinManagementDialog::loadAvailableCoinsAndTrackedCoins(
    const CoinDataStation &coinDataStation,
    const DataObserver &coinCollection) {
  QHash<QString, CoinPtr> availableCoinsFromServer{
      coinDataStation.getTrackedCoins()};
  QHash<QString, CoinPtr> trackedCoinFromCurrentCollection{
      coinCollection.getTrackedCoins()};
  qDebug() << "Collection Coin size: "
           << trackedCoinFromCurrentCollection.size();
  // Removed old Items
  ...
  while (iterQHash.hasNext()) {
    ...
    if (trackedCoinFromCurrentCollection.contains(iterQHash.key()))
      ui->listWidgetTrackedCoins->addItem(
          iterQHash.value()->getDisplayItem().get());
...
void CollectionCoinManagementDialog::moveSelectedCoinsToAvailableCoins() {
  if (ui->listWidgetTrackedCoins->selectedItems().count() < 1)
    ...
    QList<QListWidgetItem *> currentSelectedDisplayItems{
        ui->listWidgetTrackedCoins->selectedItems()};
    for (const QListWidgetItem *item : currentSelectedDisplayItems) {
      int itemRow{ui->listWidgetTrackedCoins->row(item)};
      ui->listWidgetAvailableCoins->addItem(
          ui->listWidgetTrackedCoins->takeItem(itemRow));
    }
  }
}
...
void CollectionCoinManagementDialog::checkCollectionContents() {
  if (dialogMode == Mode::AddNewCollectionMode) {
    if (ui->lineEditCollectionName->text().isEmpty())
      QMessageBox::warning(this, "Cannot create new collection",
                           "Collection name must not be empty");
    else if (ui->listWidgetTrackedCoins->count() == 0)
      QMessageBox::warning(
          this, "Cannot create new collection",
          "You must choose at least one item for collection contents");
    else {
      this->accept();
      QStringList contentsSymbol;
      for (int index{}; index < ui->listWidgetTrackedCoins->count(); index++) {
        contentsSymbol.append(
            ui->listWidgetTrackedCoins->item(index)->text().section(" ", 0, 0));
        // qDebug() << ui->listWidgetTrackedCoins->item(index)->text().section("
        // ", 0, 0);
      }
      emit requestToCreateANewCollectionWith(ui->lineEditCollectionName->text(),
                                             contentsSymbol);
    }
  } else if (dialogMode == Mode::ConfigureCurrentCollectionMode) {
    if (ui->listWidgetTrackedCoins->count() == 0)
      QMessageBox::warning(
          this, "Cannot modify the collection",
          "You must choose at least one item for collection contents");
    else {
      this->accept();
      ...
      emit requestToModifyCurrentCollectionContents(
          ui->lineEditCollectionName->text(), symbolList);
    }
  }
}

Ui::CollectionCoinManagementDialog *CollectionCoinManagementDialog::getUi()
    const {
  return ui;
}
...
void CollectionCoinManagementDialog::moveSelectedCoinsToTrackedCoins() {
  if (ui->listWidgetAvailableCoins->selectedItems().count() < 1)
    ...
    QList<QListWidgetItem *> currentSelectedDisplayItems{
        ui->listWidgetAvailableCoins->selectedItems()};
    for (const QListWidgetItem *item : currentSelectedDisplayItems) {
      int itemRow{ui->listWidgetAvailableCoins->row(item)};
      ui->listWidgetTrackedCoins->addItem(
          ui->listWidgetAvailableCoins->takeItem(itemRow));
    }
  }
}
